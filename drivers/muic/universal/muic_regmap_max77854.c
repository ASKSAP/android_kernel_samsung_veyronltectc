...
static int max77854_enable_cpen(struct regmap_desc *pdesc, int enable)
{
	int attr, value, ret = 0;
	...
	if (pdesc) {
		ret = regmap_write_value(pdesc, attr, value);
		if (ret < 0)
			pr_err("%s CONTROL2 CPEN reg write fail.\n", __func__);
		else
			_REGMAP_TRACE(pdesc, 'w', ret, attr, value);
	}
...
#if defined(CONFIG_MUIC_SUPPORT_EARJACK)
	.enable_cpen = max77854_enable_cpen,
#endif
};
