...
static struct device_attribute sec_battery_attrs[] = {
	...
	SEC_BATTERY_ATTR(cisd_batt_type),
};
...
static int sec_bat_check_wpc_temp(struct sec_battery_info *battery, int input_current)
{
	if (battery->pdata->wpc_temp_check && is_wireless_type(battery->cable_type)) {
		...
		if (battery->siop_level >= 100) {
			...
			if(battery->pdata->wpc_charging_limit_voltage[0] && is_hv_wireless_type(battery->cable_type)){
				wpc_vout_level = (!chg_limit_index) ? wpc_vout_level :
					(battery->pdata->wpc_charging_limit_voltage[chg_limit_index - 1]==9)? WIRELESS_VOUT_9V : WIRELESS_VOUT_5V;
			}
...
#if !defined(CONFIG_HV_WIRELESS_VOLTAGE_NOT_CHANGE)
...
#if defined(CONFIG_MUIC_HV) || defined(CONFIG_SUPPORT_QC30)
	} else if (!battery->chg_limit && is_hv_wire_9v_type(battery->cable_type)) {
			battery->chg_limit_recovery_cable = battery->cable_type;
				/* vbus level : 9V --> 5V */
			muic_afc_set_voltage(SEC_INPUT_VOLTAGE_5V);
...
static bool sec_bat_cisd_check(struct sec_battery_info *battery)
{
	...
	if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
		...
		if (battery->siop_level == 100 && battery->current_now > 0) {
			if (battery->charging_mode == SEC_BATTERY_CHARGING_2ND && pcisd->full_start_time <= 0) {
				pcisd->full_start_time = battery->charging_fullcharged_time;
			}
		} else {
		...
		} else if (!(pcisd->state & CISD_STATE_LEAK_D) && battery->siop_level == 100 &&
			battery->is_recharging && pcisd->recharge_count >= pcisd->recharge_count_thres) {
			now_ts = ktime_to_timespec(ktime_get_boottime());
		...
		/* discharging */
		pcisd->state &= ~(CISD_STATE_LEAK_A|CISD_STATE_LEAK_B|CISD_STATE_LEAK_C|CISD_STATE_LEAK_D);
	...
	pr_info("cisd - oo:%d, ct:%d/%d, stt:%d, cp:%d/%d, sm:%d, tm:%d, si:%d, ri:%d, cpmm:%d/%d/%d, dcpt:%d\n",
	...
	if (battery->cisd_chg_limit_enable && battery->cisd.chg_limit > 0 && !battery->store_mode) {
		...
		if ((battery->cisd.charging_disabled) &&
			...
			if (battery->cisd.over_fullcap) {
				charging_start_soc = battery->cisd.chg_limit_max;
			} else {
	...
	case CISD_DATA:
		{
			...
			i+= scnprintf(buf + i, PAGE_SIZE - i, "%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
					...
					pcisd->data[CISD_DATA_ERRCAP_HIGH]);
		}
		break;
	case CISD_BATT_TYPE:
		break;
					...
					const char *buf, size_t count)
{
	...
	case CISD_DATA:
		{
			...
			if (sscanf(buf, "%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
					...
					&temp_data[9], &temp_data[10]) == CISD_DATA_MAX) {
				struct cisd *pcisd = &battery->cisd;
	...
	case CISD_BATT_TYPE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			if (battery->use_cisd && battery->cisd_chg_limit_enable && x == 0) {
				battery->cisd.chg_limit = battery->cisd.chg_limit_max;
				pr_info("%s: update cisd chg_limit(%d)\n", __func__,
					battery->cisd.chg_limit);
				wake_lock(&battery->monitor_wake_lock);
				queue_delayed_work(battery->monitor_wqueue, &battery->monitor_work, 0);
		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_u32(np, "battery,cisd_chg_limit_max",
		&battery->cisd.chg_limit_max);
	if (ret) {
		pr_info("%s : cisd_chg_limit_max is Empty\n", __func__);
		battery->cisd.chg_limit_max = 60;
	}
	...
	if (pdata->wpc_temp_check) {
		...
		pdata->wpc_charging_limit_voltage = kzalloc(sizeof(unsigned int) * len, GFP_KERNEL);
		if (!pdata->wpc_high_temp ||
			!pdata->wpc_high_temp_recovery ||
			!pdata->wpc_charging_limit_current ||
			!pdata->wpc_charging_limit_voltage)
			return 1;
		...
		for (i = 0; i < len; i++) {
			...
			ret = of_property_read_u32_index(np,
				"battery,wpc_charging_limit_voltage", i, &temp);
			pdata->wpc_charging_limit_voltage[i] = (int)temp;
			if (ret){
				pdata->wpc_charging_limit_voltage[i] = 0;
				pr_info("%s : wpc_charging_limit_voltage is Empty\n", __func__);
			}
		}

		pr_info("%s: wpc_high_temp_size(%d)\n", __func__, pdata->wpc_high_temp_size);
		for (i = 0; i < len; i++) {
			pr_info("%s: index(%d) - wpc_high_temp(%d), wpc_high_temp_recovery(%d), wpc_charging_limit_current(%d), wpc_charging_limit_voltage(%d)\n",
				__func__, i,
				pdata->wpc_high_temp[i],
				pdata->wpc_high_temp_recovery[i],
				pdata->wpc_charging_limit_current[i],
				pdata->wpc_charging_limit_voltage[i]);
		}
...
static void sec_battery_cisd_init(struct sec_battery_info *battery)
{
	...
	battery->cisd.lcd_off_delay_time = 16200; /* 270 min */
