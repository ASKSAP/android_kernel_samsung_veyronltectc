...
#define ATD_HS_DEV_NAME "/dev/ttyHS2"
...
static int msm_hs_remove(struct platform_device *pdev)
{
	...
	if(pdata == NULL) {
		pr_err("Invalid msm_serial_hs_platform_data\n");
		return -EINVAL;
	}
...
void msm_hs_request_clock_off(struct uart_port *uport)
{
	...
	if (atomic_read(&msm_uport->client_count) <= 0) {
		MSM_HS_WARN("%s(id:%u): ioctl count -ve, client check voting",
			__func__, pdev->id);
		return;
	}
	...
	MSM_HS_INFO("%s(id:%u):DISABLE UART CLOCK: ioc %d\n",
			__func__, pdev->id, atomic_read(&msm_uport->client_count));
}
...
void msm_hs_request_clock_on(struct uart_port *uport)
{
	...
	MSM_HS_INFO("%s(id:%u):ENABLE UART CLOCK: ioc %d\n",
		__func__, pdev->id, client_count);
	msm_hs_resource_vote(UARTDM_TO_MSM(uport));

	if (msm_uport->pm_state != MSM_HS_PM_ACTIVE) {
		MSM_HS_WARN("%s(id:%u): %p runtime PM callback not invoked",
			__func__, pdev->id, uport->dev);
		msm_hs_pm_resume(uport->dev);
	}
...
static void msm_hs_pm_suspend(struct device *dev)
{
	...
	MSM_HS_DBG("%s(), id(%u): return suspend\n", __func__, pdev->id);
	return;
err_suspend:
...
static int msm_hs_pm_resume(struct device *dev)
{
	...
	MSM_HS_DBG("%s(), id(%u): return resume\n", __func__, pdev->id);
	return 0;
err_resume:
	pr_err("%s(): invalid uport", __func__);
	return 0;
...
static int msm_hs_probe(struct platform_device *pdev)
{
	...
	struct msm_serial_hs_platform_data *pdata = pdev->dev.platform_data;
	unsigned long data;
	char ipc_log_name[MAX_LOG_NAME_LEN];
	...
	if (pdev->id)
		snprintf(ipc_log_name, MAX_LOG_NAME_LEN, "msm_serial_hs%d", pdev->id);
	else
		snprintf(ipc_log_name, MAX_LOG_NAME_LEN, "msm_serial_hs");
