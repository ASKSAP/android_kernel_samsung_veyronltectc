			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	*/
	...
	mutex_init(&vdd_data.vdd_hall_ic_sd_enable_lock);
	mutex_init(&vdd_data.vdd_hall_ic_sd_scm_lock); 
			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	vdd->line_split = of_property_read_bool(np, "samsung,line_split");
	LCD_INFO("line_split is %s\n", vdd->line_split ? "enabled" : "disabled");
...
*/
#define WAIT_MAX_FRAME 58
#define WAIT_ONE_FRMAE 17000
	...
	*/
	...
	if (mdss_panel_attached(DISPLAY_1) && mdss_panel_attached(DISPLAY_2)) {
		...
		if (vdd_data.vdd_blank_mode[DISPLAY_1] == FB_BLANK_UNBLANK &&
			...
			mutex_lock(&vdd_data.vdd_hall_ic_sd_enable_lock);

			if (mdp5_data->sd_enabled) {
				while (wait_frame <= WAIT_MAX_FRAME) {
					usleep_range(WAIT_ONE_FRMAE, WAIT_ONE_FRMAE);

					if (!mdp5_data->sd_enabled) {
						LCD_ERR("wait_ms : %d\n", (wait_frame * WAIT_ONE_FRMAE) / 1000);
						break;
					}
				...
				if (wait_frame > WAIT_MAX_FRAME)
					LCD_ERR("fail to wait normal display\n");
			}
			...
			mutex_unlock(&vdd_data.vdd_hall_ic_sd_enable_lock);
			...
			if (vdd_data.ctrl_dsi[DISPLAY_1]->panel_mode == DSI_CMD_MODE &&
					!flip_not_refresh) {
				if (is_boot_recovery || poweroff_charging)
