				...
				struct mdp_display_commit *data)
{
...
#if defined(CONFIG_FB_MSM_MDSS_SAMSUNG)
	struct samsung_display_driver_data *vdd = mdss_samsung_get_vdd(mdp5_data->ctl);
#endif
	...
	if (!mdp5_data->sd_enabled && sd_in_pipe) {
		mutex_lock(&vdd->vdd_hall_ic_sd_enable_lock);
		mutex_lock(&vdd->vdd_hall_ic_sd_scm_lock);
		if (!mdss_get_sd_client_cnt())
		...
		mutex_unlock(&vdd->vdd_hall_ic_sd_scm_lock);
		mutex_unlock(&vdd->vdd_hall_ic_sd_enable_lock);
	}
