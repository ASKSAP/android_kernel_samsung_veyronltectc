	...
	struct device_attribute *attr, char *buf)
{
	...
	if (data->gyro_st_result.result1 != 0) {
		pr_info("[FACTORY]: %s - fifo zro fail (%d, %d, %d)\n",
			__func__, gyro_fifo_avg[0],
			gyro_fifo_avg[1], gyro_fifo_avg[2]);
	...
	if (data->gyro_st_result.result2 != 0) {
		pr_info("[FACTORY]: %s - ST fail (%d, %d, %d)\n",
