				 ...
				 int req, bool is_dapm)
{
	...
	case MICB_DISABLE:
		...
		if ((tasha->micb_ref[micb_index] == 0) &&
			...
			snd_soc_update_bits(codec, micb_reg, 0xC0, 0xd0);
#else
			snd_soc_update_bits(codec, micb_reg, 0xC0, 0x00);
#endif
			if (post_off_event)
...
#if defined (CONFIG_SEC_VEYRONLTE_PROJECT)
	snd_soc_update_bits(codec, WCD9335_ANA_MICB1, 0xFF, 0xd0);
	snd_soc_update_bits(codec, WCD9335_ANA_MICB2, 0x3F, vout_ctl_2);
	snd_soc_update_bits(codec, WCD9335_ANA_MICB3, 0xFF, 0xd0);
#else
